...
requirejs(["jquery", "TestPlugin", "Tabs", "Greenify", "Common_Widgets", "bootstrap4-tagsinput"], function (jQuery, TestPlugin, TabsModule, GreenifyModule, CommonWidgetsModule, bootstrap4_tagsinput) {
    ...
    function init() {
        ...
        var inputElements = jQuery('select.tagsinputtarget');
        ...
        var modelTablesContainer = jQuery('#modelTablesContainer');
    ...
    function attachEventHandlers() {
        ...
        body.on("click", '#testSetting1Form .pendingActionButton', function () {
            ...
            var settingValInput = jQuery('#testSetting1Input');
            var settingTagsinput = settingValInput.parent().find('.bootstrap-tagsinput');
            //save keywords
            var settingValue = settingValInput.tagsinput('items'); //because result is an array, join it to get a comma-separated list
            if (TestPlugin.Utilities.TypeChecker.isArray(settingValue)) {
                settingValue = settingValue.join();
            }
            var settingID = settingValInput.data("setting-id");
    ...
    function testUserAjax() {
        ...
        var payload = new TestPlugin.WPAjaxPayload(TestPlugin.PluginData.ajaxEndpoints.user.getTestUserAjaxResponse, jsonData);
        return jQuery.ajax({
    ...
    function testAdminAjax() {
        var jsonData = JSON.stringify([1, 2, 3]);
        var payload = new TestPlugin.WPAjaxPayload(TestPlugin.PluginData.ajaxEndpoints.admin.getTestAdminAjaxResponse, jsonData, TestPlugin.PluginData.nonce);
        return jQuery.ajax({
            data: payload.prepare(),
            success: function (data) {
            },
    ...
    function saveSettingModel(settingID, settingVal) {
        var jsonData = {
            'object': 'Setting',
            'fieldsvalues': { 'settingvalue': settingVal },
            'id': settingID
        };
        var payload = new TestPlugin.WPAjaxPayload(TestPlugin.PluginData.ajaxEndpoints.admin.saveObjectFields, JSON.stringify(jsonData), TestPlugin.PluginData.nonce).prepare();
        return jQuery.ajax({
            data: payload
        });
    }
